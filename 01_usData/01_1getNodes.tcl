...
# vmd -dispdev text -e 01_getNodes.tcl -args "deca-ala_H_v.psf" "../../../../0_helix_f100.coor" "../../../../tmd_out_all.dcd" "../../../../tmd_out_all.xst" 50 32 25 10 15 20 6 10 no yes 180.1 0.4 new
...
if {$argc != 19} {
  ...
  puts "15: processing mode (new/modified)"
...
if {$procMode ne "new" && $procMode ne "modified"} {
...
if {$procMode eq "new" || $procMode eq "continued"} {
  exit
}

set fileIn "01_allWinFrameIndex.txt"
if [catch {open $fileIn r} FIN] {
  puts "ERROR: Can not open $fileIn"
  exit
}
...
while {[gets $FIN line] >= 0} {
  set winIndexPair($numWinIndexPair) $line
...
for {set i0 0} {$i0 < $numWinIndexPair} {incr i0 1} {
  animate write namdbin "01_[lindex $winIndexPair($i0) 0].coor" beg [lindex $winIndexPair($i0) 1] end [lindex $winIndexPair($i0) 1] sel $allSel0 waitfor all 0
}
...
if {$useXst eq "yes"} {
  ...
  if {[gets $FIN line] >= 0} {
    if {$line eq "#\$LABELStep o_x o_y o_z"} {
      puts "in vacuum???"
      set enviroment 7
    } elseif {$line eq "#\$LABELS step a_x a_y a_z b_x b_y b_z c_x c_y c_z o_x o_y o_z s_x s_y s_z s_u s_v s_w"} {
      puts "in solution"
      set enviroment 1
    } elseif {$line eq "#\$LABELS step o_x o_y o_z"} {
      puts "in vacuum"
      set enviroment 0
    } else {
