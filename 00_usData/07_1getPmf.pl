...
$fileIn = "01_allNodesM2.txt";
if(open(FIN, "$fileIn") != 1)
{
  print "WARNING: Can not open $fileIn\n";
...
#         $targVecEndPoint[$i[0]] = $lastSampCenOnOriVec[$i[0]];;
    ...
    for($i[0] = $first, $i[1] = $last + 1; $i[0] < $i[1]; $i[0]++)
    {
      ...
      {
        $binNum = int(($targVecEndPoint[$i[0]] - $lastSampCenOnNextVec[$i[0]-1]) * $binDen + 1);
        if($binNum == 1)
        {
          $binNum++;
        }
        system("$wham 07_2data4whamS_r$i[0].txt 0 $targVecEndPoint[$i[0]] $binNum $tolE $maxIter $tempK $kBN > 07_pmfR$i[0].tmp");
#         system("$wham 07_2data4whamS_r$i[0].txt $lastSampCenOnNextVec[$i[0]-1] $targVecEndPoint[$i[0]] $binNum $tolE $maxIter $tempK $kBN > 07_pmfR$i[0].tmp");
      }
